...
[`.github/regen-ci-deps-lock.sh`](regen-ci-deps-lock.sh) running
inside the CI container (or via the `Regen CI-deps Lock`
workflow_dispatch for maintainers without local Docker), and must be
