...
def free_form_deform(
    ...
    *,
    ...
    ``physicsnemo.geometry.free_form_deform(mesh, ...)``.
    ...
    control_displacements : torch.Tensor
        ...
        node, with shape ``(n_1, ..., n_D, D)`` for the shared spatial
        dimension ``D``. The float32 or float64 dtype and device must match the
        input points, including every DomainMesh component. Each axis needs at
        least two nodes for ``"bernstein"`` and the node-interpolating bases,
        and four for ``"bspline"``.
    origin : torch.Tensor, sequence of float, or None, optional
        Minimum corner of the lattice box with shape ``(D,)``. ``None`` uses
        the minimum corner of the Mesh bounds or the combined DomainMesh
        component bounds. For repeated GPU calls with an explicit box, create
    ...
    extent : torch.Tensor, sequence of float, or None, optional
        ...
        the box from ``origin`` to the maximum corner of the Mesh or combined
        DomainMesh component bounds.
    ...
    point_weights : str, tuple[str, ...], torch.Tensor, or None, optional
        ...
        each value must match that component's point count. All weights must
        match the point device. Floating weights must also match the point
    ...
    if isinstance(mesh, DomainMesh):
        if point_weights is not None and not isinstance(point_weights, (str, tuple)):
            raise TypeError(
                "physicsnemo.geometry.free_form_deform point_weights must be a "
                "common point_data key/path for DomainMesh inputs, not a raw tensor"
            )
        ...
        def apply_field(
            ...
            box_origin, box_extent = _default_lattice_box(points, origin, extent)
        ...
        return _deform_domain(
            ...
            reference=control_displacements,
            reference_name="control_displacements",
