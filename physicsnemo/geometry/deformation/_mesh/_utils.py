...
def _deform_domain(
    ...
    *,
    ...
    """Apply one deformation field to all DomainMesh components together."""

    components: list[tuple[str, Mesh]] = [("interior", domain.interior)]
    components.extend(
        (f"boundaries[{name!r}]", domain.boundaries[name])
        for name in domain.boundaries.keys()
    )
    ...
    return DomainMesh(
        ...
        boundaries={
            ...
            for index, name in enumerate(domain.boundaries.keys(), start=1)
        },
        global_data=domain.global_data.clone(),
    )
