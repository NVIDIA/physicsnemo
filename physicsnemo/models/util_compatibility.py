...
def convert_ckp_apex(
    ...
    apex_in_ckp = ("use_apex_gn" in ckp_args_dict["__args__"].keys()) and (ckp_args_dict["__args__"]["use_apex_gn"])
    apex_in_workflow = (model_args is not None) and ("use_apex_gn" in model_args.keys()) and (model_args["use_apex_gn"])
        ...
        for key, value in model_dict.items():
            ...
            for norm_layer in ["norm0", "norm1", "norm2", "aux_norm"]:
                if f"{norm_layer}.gn.weight" in key:
                    ...
                    new_key = key.replace(
                        f"{norm_layer}.gn.bias", f"{norm_layer}.bias"
                    )
                    filtered_state_dict[new_key] = value  # Duplicate bias
    else:
