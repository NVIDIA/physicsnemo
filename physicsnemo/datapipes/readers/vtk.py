...
class VTKReader(Reader):
    ...
    def __init__(
        ...
        *,
        ...
        cache : DatasetCache, optional
            ...
            sample per file type otherwise). Assumes the dataset is
            immutable; call ``cache.clear()`` after adding or renaming
            files.
    ...
    def _get_file_by_extension(self, directory: Path, extension: str) -> Optional[Path]:
        ...
        identity = (
            f"{directory}::{extension}::{','.join(sorted(self.exclude_patterns))}"
        )
        found = self._cached("vtk-file/v1", identity, _scan)
