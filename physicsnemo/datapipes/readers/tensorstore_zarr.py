    ...
    def __init__(
        ...
        *,
        ...
        cache : DatasetCache, optional
            ...
            A cached spec is trusted without re-reading the array's
            metadata (``assume_metadata=True``), so the dataset must be
            immutable while cached: rewriting an array with a different
            shape or dtype requires ``cache.clear()`` or a fresh
            ``disk_dir``, or reads return wrong data without error.
