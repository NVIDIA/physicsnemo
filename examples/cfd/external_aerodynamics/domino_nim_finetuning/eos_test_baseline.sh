...
#SBATCH -J coreai_modulus_cae-modulus:test_baseline_500
#SBATCH -t 00:15:00
#SBATCH -p batch
#SBATCH -N 1
#SBATCH --dependency=singleton
#SBATCH -o ./sbatch_logs_baseline_test/multi/%x_%j.out
#SBATCH -e ./sbatch_logs_baseline_test/multi/%x_%j.err
# Parameterize the checkpoint epoch
CHECKPOINT_EPOCH=${1:-200}
...
# train the model
RUN_CMD="python -u src/test_baseline.py exp_tag=21 model.model_type=combined model.activation=relu eval.checkpoint_name=DoMINO.0.${CHECKPOINT_EPOCH}.pt eval.save_path=/lustre/rranade/modulus_dev/data/drivaer_data_baseline_test_predicted_${CHECKPOINT_EPOCH}"
# nohup torchrun --nnodes 1 --nproc_per_node 8 src/train.py exp_tag=1 model.model_type=combined model.surface_points_sample=54000 model.volume_points_sample=54000 model.loss_function.loss_type=mse model.activation=relu > output_train_1_baseline.log &
