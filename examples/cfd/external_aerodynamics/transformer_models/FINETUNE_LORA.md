...
This is a companion to `train.py`: same model, same data pipeline, same
`conf/` groups — only the entry point (`src/finetune.py`) and config
(`conf/finetune_lora.yaml`) are new. `train.py` is unchanged.
...
## Config (`conf/finetune_lora.yaml`)
...
- `peft.rank` / `peft.alpha`: adapter capacity / scaling.
- Point the `data` group at your small dataset (see `conf/data/{core,surface}.yaml`).
