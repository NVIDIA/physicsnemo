...
"""Fit a lifted springback sheet to a curved engineering panel.

The target and source are built directly with PyTorch. Their resolutions are
intentionally different, so the source vertices project into the target
triangles instead of sharing target features. ``Mesh.shrinkwrap`` fits the
movable sheet vertices while a boolean point-weight mask holds the root strip.
...
The example validates the Torch CPU backend. When CUDA is available, it also
checks the Warp result against Torch before rendering the same deterministic
Torch result.
"""
...
def _panel_normals(
    ...
    epsilon = 1.0e-3
    width_tangent = (
        _panel_surface(width_fraction + epsilon, span_fraction)
        - _panel_surface(width_fraction - epsilon, span_fraction)
    ) / (2.0 * epsilon)
    span_tangent = (
        _panel_surface(width_fraction, span_fraction + epsilon)
        - _panel_surface(width_fraction, span_fraction - epsilon)
    ) / (2.0 * epsilon)
    return F.normalize(
...
def build_case(
    ...
    target_span, target_width = _parameter_grid(
        44,
        31,
        width_limit=1.0,
        span_limit=1.0,
    ...
    target = Mesh(
        points=_panel_surface(target_width, target_span).reshape(-1, 3),
        cells=_structured_cells(44, 31, device=device),
    )

    source_span, source_width = _parameter_grid(
        19,
        25,
        width_limit=0.96,
        span_limit=0.985,
    ...
    nominal = _panel_surface(source_width, source_span)
    normals = _panel_normals(source_width, source_span)
    ...
    source = Mesh(
        ...
        cells=_structured_cells(19, 25, device=device),
    )
...
def run_backends(
    ...
    torch_residual = _surface_residual(
        target,
        result,
        movable,
        implementation="torch",
    ...
    if torch.cuda.is_available():
        ...
        warp_residual = _surface_residual(
            target_cuda,
            warp_result,
            movable_cuda,
            implementation="warp",
...
def _camera_position(mesh: Mesh) -> list[list[float]]:
...
def render_case(
    ...
    _add_fixed_root(plotter, source, movable)
    plotter.add_text(
    ...
    _add_fixed_root(plotter, result, movable)
    plotter.add_text(
    ...
    camera = _camera_position(target)
...
def main(output: Path) -> None:
    """Run panel conformance and save its comparison figure."""

    target, source, movable = build_case()
    result = run_backends(target, source, movable)
    render_case(target, source, result, movable, output)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("shrinkwrap_panel_conformance.png"),
        help="PNG path (default: ./shrinkwrap_panel_conformance.png)",
    )
