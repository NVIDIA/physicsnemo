...
def stack_unified_observations(data: list[UnifiedObservation]) -> UnifiedObservation:
    """Concatenate observations"""
    tensors = []
    offset = 0

    offsets = []
    hpx_level = data[0].hpx_level
    for obs in data:
        assert obs.hpx_level == hpx_level
        if obs.offsets is None:
            offsets.append(torch.tensor(offset))
        else:
            offsets.append(offset + obs.offsets)
        ...
        offset += obs.obs.shape[0]

    obs, time, float_meta, int_meta = list(zip(*tensors))

    return UnifiedObservation(
        obs=torch.cat(obs),
        time=torch.cat(time),
        float_metadata=torch.cat(float_meta),
        int_metadata=torch.cat(int_meta),
