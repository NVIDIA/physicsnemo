...
network depending on a given conditionning variable. For temporal
...
* A dataset of yearly HDF5 files at one-hour resolution. For more details, refer to the section
  ...
  other resolutions can work too.
...
[Hydra](https://hydra.cc/), which is found in the `config` directory. The full configuration for
training of the original model is
...
Test the training by running the `train.py` script using the "lite" configuration file on a
system with a GPU:
...
eight nodes with eight GPUs each, for a total of 64 GPUs, start a distributed compute job (for example,
