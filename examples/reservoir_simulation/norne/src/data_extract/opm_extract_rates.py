...
# clip_and_convert_to_float3, Make_correct, Split_Matrix
...
def read_compdats2(filename, file_path):
    ...
    gas_injectors, water, oil = extract_tuples(
        injector_gas, injector_water, well_namesoil, data
    )
...
def historydata(timestep, steppi, steppi_indices, source_dir):
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_OIL_SEC1
            ):  # Skip header lines; this RSM section starts at line 47873
                continue
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_OIL_SEC2
            ):  # Skip header lines; this RSM section starts at line 48743
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_OIL_SEC3
            ):  # Skip header lines; this RSM section starts at line 49613
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_OIL_SEC4
            ):  # Skip header lines; this RSM section starts at line 50483
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_WATER_SEC1
            ):  # Skip header lines; this RSM section starts at line 40913
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_WATER_SEC2
            ):  # Skip header lines; this RSM section starts at line 41783
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_WATER_SEC3
            ):  # Skip header lines; this RSM section starts at line 42653
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_WATER_SEC4
            ):  # Skip header lines; this RSM section starts at line 43523
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_GAS_SEC1
            ):  # Skip header lines; this RSM section starts at line 54833
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_GAS_SEC2
            ):  # Skip header lines; this RSM section starts at line 55703
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_GAS_SEC3
            ):  # Skip header lines; this RSM section starts at line 56573
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_GAS_SEC4
            ):  # Skip header lines; this RSM section starts at line 57443
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_WINJ_SEC1
            ):  # Skip header lines; this RSM section starts at line 72237
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
    ...
    with open(os.path.join(source_dir, "NORNE_ATW2013.RSM")) as f:
        for i, line in enumerate(f):
            if (
                i < _RSM_GINJ_SEC1
            ):  # Skip header lines; this RSM section starts at line 73977
                continue
            if "---" in line:  # Stop reading when '---' is found
                break
            lines.append(line)
...
def get_model_hash(model):
    """Compute a SHA-256 hash of a model's state dict for change detection.

    Parameters
    ----------
    model : torch.nn.Module
        PyTorch model whose parameters will be hashed.

    Returns
    -------
    str
        Hexadecimal SHA-256 digest of the serialised state dict.
    """
...
def write_RSM(data, Time, Name, well_names):
    """Write multi-header well rate data to an Excel spreadsheet.

    Parameters
    ----------
    data : np.ndarray
        Rate data of shape ``(n_timesteps, n_wells * n_groups)`` in
        column order ``[WOPR wells, WWPR wells, WGPR wells]``.
    Time : np.ndarray
        1-D array of time values written to the first column.
    Name : str
        Output file path without the ``.xlsx`` extension.
    well_names : list[str]
        Ordered list of producer well names used for column labels.
...
def normalize_tensors_adjusted(tensor_dict):
    """Min-max normalise each tensor in a dict and add a small perturbation.

    Each tensor is scaled to ``[0, 1]`` and mixed with a clamped Gaussian
    perturbation (mean 0.1, std 0.01) to avoid degenerate zero inputs.

    Parameters
    ----------
    tensor_dict : dict
        Mapping of string keys to ``torch.Tensor`` values.

    Returns
    -------
    dict
        New dict with the same keys mapped to normalised ``torch.Tensor``
        values.
    """
    normalized_dict = {}
...
def safe_mean_std(data):
    """Compute mean and sample std, returning None for std if it is infinite.

    Parameters
    ----------
    data : np.ndarray
        Input array from which statistics are computed over all elements.

    Returns
    -------
    tuple[float, float | None]
        ``(mean, std)`` where `std` is ``None`` if the value is infinite.
    """
...
def safe_rmtree(path, retries=3, delay=1):
    """Remove a directory tree with retry logic for busy filesystems.
    ...
    delay : int, optional
        Seconds to wait between attempts when the resource is busy.
...
def InitializeLoggers(cfg: DictConfig) -> tuple[DistributedManager, PythonLogger]:
    ...
    logger = PythonLogger(
        name=f"PhysicsNeMo Reservoir_Characterisation{dist_manager.rank}"
    )
...
def on_rm_error(func, path, exc_info):
    """Error handler for `shutil.rmtree` that retries after granting write access.

    Parameters
    ----------
    func : callable
        The failed filesystem function (e.g., ``os.remove``).
    path : str
        Path of the file or directory that caused the error.
    exc_info : tuple
    ...
    try:
        # Clear the read-only attribute so the retry can delete the file. On
        # Windows this removes the read-only flag (the documented rmtree fix);
        # on POSIX it enables owner write.
        os.chmod(path, stat.S_IWRITE)
...
def check_and_remove_dirs(directories, response):
    """Conditionally remove a list of directories based on a user response.

    Parameters
    ----------
    directories : list[str]
        Paths of directories to potentially remove.
    response : str
        Pre-collected user answer; ``'yes'`` triggers removal, anything
        else skips the directory.
...
def are_models_equal(model1, model2):
    """Check whether two PyTorch models have identical parameter values.
    ...
    -------
    bool
        ``True`` if every parameter tensor is element-wise equal, else ``False``.
    """
    return all(
