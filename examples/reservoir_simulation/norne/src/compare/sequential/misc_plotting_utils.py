...
"""
=====================================================================
                    SEQUENTIAL PLOTTING UTILITIES - CORE FUNCTIONS
=====================================================================

This module provides core plotting utilities for sequential processing of FVM
...
def assign_faults(well_indices, nx, ny, nz, well_amount, data):
    ...
    well_amount : list or set
        Ordered collection of fault names to align with data values.
...
def read_compdats2(filename, file_path):
    """
    Parse a DATA file to classify wells as gas injectors, water injectors, or oil producers.

    Parameters
    ----------
    filename : str
        Path to the Eclipse DATA file containing WELSPECS, WCONINJE, and WCONHIST keywords.
    file_path : str
        Path to the RSM-style file listing WOPR producer well names.

    Returns
    -------
    gas_injectors : list of tuple
        COMPDAT tuples for gas injector wells with (i, j, well_name).
    oil : list of tuple
        COMPDAT tuples for oil producer wells with (i, j, well_name).
    water : list of tuple
        COMPDAT tuples for water injector wells with (i, j, well_name).
    """
    with open(filename) as file:
...
def historydata(timestep, steppi, steppi_indices):
    ...
    lines = []
    with open(to_absolute_path("../simulator_data/NORNE_ATW2013.RSM")) as f:
