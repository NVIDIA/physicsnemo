...
def Plot3DNorne(
    ...
    """
    Produce a 4-panel 3-D and 2-D visualisation of a Norne field property.
    ...
    cgrid : np.ndarray
        Corner-point grid coordinate array of shape (n_active, 7).
    varii : str
        Property label; controls titles and colourbar labels (e.g. 'perm', 'oil').
    injectors : list of tuple
        Water injector location tuples (x, y, ..., well_name at index 8).
    producers : list of tuple
        Producer location tuples (x, y, ..., well_name at index 8).
    gas_injectors : list of tuple
        Gas injector location tuples (x, y, ..., well_name at index 8).
    ...
    if np.any(np.isnan(Pressz)):
        warnings.warn(
            "NaN values detected in Pressz; replacing colors with 1.", stacklevel=2
        )
...
def Plot_all_layesr(nx, ny, nz, see, injectors, producers, gas_injectors, varii):
    """
    Plot each model layer as a 2-D pcolormesh subplot in a 5x5 grid figure.
    ...
    ny : int
        Grid dimension in the Y direction.
    nz : int
        Grid dimension in the Z direction (number of subplots).
    see : np.ndarray
        Flattened 1-D field array of length nx*ny*nz; zeros converted to NaN.
    injectors : list of tuple
        Water injector location tuples.
    producers : list of tuple
        Producer location tuples.
    gas_injectors : list of tuple
        Gas injector location tuples.
    varii : str
        Property label controlling titles and colourbar labels.
    ...
    for i in range(nz):
        ...
        plt.gca().set_xticks([])
        plt.gca().set_yticks([])
        Add_marker3(plt, XX, YY, injectors, producers, gas_injectors)
    if varii == "perm":
        plt.suptitle(
...
def Add_marker2(plt, XX, YY, injectors, producers, gas_injectors):
    """
    Overlay large well-location scatter markers and labels on a 2-D plot.

    Parameters
    ----------
    plt : module
        The matplotlib.pyplot module (or compatible interface).
    XX : np.ndarray
        2-D mesh of X coordinates.
    YY : np.ndarray
        2-D mesh of Y coordinates.
    injectors : list of tuple
        Water injector tuples; index 0 is x, 1 is y, 8 is well name.
    producers : list of tuple
        Producer tuples; index 0 is x, 1 is y, 8 is well name.
    gas_injectors : list of tuple
        Gas injector tuples; index 0 is x, 1 is y, 8 is well name.
...
def Add_marker3(plt, XX, YY, injectors, producers, gas_injectors):
    """
    Overlay small well-location scatter markers and labels on a 2-D subplot.
