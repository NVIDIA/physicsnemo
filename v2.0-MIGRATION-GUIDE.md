...
Capabilities include discrete calculus (gradient, divergence, curl,
Laplace-Beltrami via DEC and least-squares methods, with intrinsic and
extrinsic variants), Gaussian and mean curvature, subdivision (linear, Loop,
Butterfly), mesh repair, BVH spatial queries, topology analysis, visualization
(`mesh.draw()`), and a library of ready-made primitive meshes. The only core
dependencies are PyTorch and TensorDict; PyVista is optional for I/O and 3D
visualization. See `physicsnemo/mesh/README.md` for the full feature matrix.
