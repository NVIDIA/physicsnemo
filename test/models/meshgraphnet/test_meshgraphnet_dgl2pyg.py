    ...
    with modify_environment(PHYSICSNEMO_FORCE_TE=TE_FORCE_VAL):
        ...
        for i in range(batch_size):
            ...
            node_features = torch.randn(
                num_nodes_per_graph, input_dim_nodes, device=device
            )
            edge_features = torch.randn(
                num_edges_per_graph, input_dim_edges, device=device
            )
...
def test_meshgraphnet_batched_equivalence(device, pytestconfig):
    ...
    with modify_environment(PHYSICSNEMO_FORCE_TE=TE_FORCE_VAL):
        ...
        # Forward pass with batched DGL graph.
        output_dgl = model(
            batched_node_features, batched_edge_features, batched_dgl_graph
        )

        # Forward pass with batched PyG graph.
        output_pyg = model(
            batched_node_features, batched_edge_features, batched_pyg_graph
        )
