...
from physicsnemo.experimental.models.dit.layers import DiTBlock, TokenizerModuleBase, DetokenizerModuleBase, AttentionModuleBase
...
class CustomDetokenizer(DetokenizerModuleBase):
    ...
    def __init__(self, out_channels: int, input_size: Tuple[int, int], hidden_size: int, patch_size: int):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        x = x.transpose(1, 2).reshape(-1, self.hidden_size, self.input_size[0]//self.patch_size, self.input_size[1]//self.patch_size)
...
@pytest.mark.parametrize("device", ["cuda:0"])
@pytest.mark.parametrize("tokenizer", [CustomTokenizer(in_channels=3, hidden_size=64, patch_size=4), "patch_embed_2d"])
@pytest.mark.parametrize("detokenizer", [CustomDetokenizer(out_channels=4, input_size=(16, 16), hidden_size=64, patch_size=4), "proj_reshape_2d"])
@pytest.mark.parametrize("attention_backend", [CustomAttention(hidden_size=64, num_heads=2), "timm"])
