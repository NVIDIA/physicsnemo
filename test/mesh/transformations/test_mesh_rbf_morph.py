...
"""Mesh and DomainMesh integration tests for thin-plate-spline RBF morphing."""
...
from physicsnemo.mesh.transformations.deform import rbf_morph
...
def test_rbf_morph_namespace_is_canonical():
    ...
    assert deform_module.rbf_morph is rbf_morph
    assert not hasattr(transformations, "rbf_morph")
    assert not hasattr(geometric_module, "rbf_morph")


def test_mesh_and_domain_rbf_morph_signatures_are_introspectable():
    for method in (Mesh.rbf_morph, DomainMesh.rbf_morph):
...
def test_mesh_rbf_morph_method_and_function_interpolate_controls_exactly():
    mesh = _triangle_mesh()
    controls, displacements = _controls_and_displacements()
    expected = mesh.points + displacements

    method_output = mesh.rbf_morph(
        controls,
        displacements,
        polynomial=True,
        smoothing=0.0,
...
def test_mesh_rbf_morph_preserves_data_and_invalidates_only_geometry_cache():
    mesh = _triangle_mesh()
...
def test_mesh_rbf_morph_resolves_nested_point_weight_key():
    mesh = _triangle_mesh()
...
def test_mesh_rbf_morph_preserves_autograd_through_fit_and_evaluation():
    points = torch.tensor(
...
def test_mesh_rbf_morph_has_actionable_boundary_validation():
    mesh = _triangle_mesh()
    controls, displacements = _controls_and_displacements()

    with pytest.raises(TypeError, match="control_points"):
        mesh.rbf_morph("handles", displacements, implementation="torch")
    with pytest.raises(TypeError, match="control_displacements"):
        mesh.rbf_morph(controls, "motions", implementation="torch")
...
def test_domain_rbf_morph_shared_controls_and_common_point_weight_key(point_weights):
    domain = _domain_with_coincident_points()
    controls, displacements = _controls_and_displacements()

    output = domain.rbf_morph(
        controls,
        displacements,
...
def test_domain_rbf_morph_resolves_common_nested_point_weight_path():
    domain = _domain_with_coincident_points()
    ...
    nested = domain.rbf_morph(
        controls,
        displacements,
        point_weights=("motion", "weight"),
        implementation="torch",
    )
    flat = domain.rbf_morph(
        controls,
        displacements,
...
def test_domain_rbf_morph_missing_common_key_names_failing_component():
    domain = _domain_with_coincident_points()
...
def test_domain_rbf_morph_rejects_raw_point_weight_tensor():
    domain = _domain_with_coincident_points()
    controls, displacements = _controls_and_displacements()
...
def test_domain_rbf_morph_clones_domain_global_data():
    domain = _domain_with_coincident_points()
    controls, displacements = _controls_and_displacements()
    original_reynolds = domain.global_data["reynolds"].clone()

    output = domain.rbf_morph(controls, displacements, implementation="torch")
...
def test_domain_rbf_morph_evaluates_combined_components_once(monkeypatch):
    domain = _domain_with_coincident_points()
    ...
    controls, displacements = _controls_and_displacements()
    expected = {
        "interior": domain.interior.rbf_morph(
            controls,
            displacements,
            point_weights="marker",
            implementation="torch",
    ...
    original = deform_module.rbf_morph_points
    calls: list[torch.Tensor] = []
    options: list[tuple[str, bool, float]] = []

    def counted_rbf_morph_points(points, *args, **kwargs):
        calls.append(points)
        options.append((kwargs["kernel"], kwargs["polynomial"], kwargs["smoothing"]))
        return original(points, *args, **kwargs)

    monkeypatch.setattr(deform_module, "rbf_morph_points", counted_rbf_morph_points)
    output = domain.rbf_morph(
        controls,
        displacements,
        point_weights="marker",
...
def test_domain_rbf_morph_single_component_avoids_concatenation(monkeypatch):
    domain = DomainMesh(interior=_triangle_mesh())
    deform_module = importlib.import_module("physicsnemo.nn.functional.geometry.deform")
    original = deform_module.rbf_morph_points
    received_points: list[torch.Tensor] = []

    def inspect_rbf_morph_points(points, *args, **kwargs):
        received_points.append(points)
        return original(points, *args, **kwargs)

    monkeypatch.setattr(deform_module, "rbf_morph_points", inspect_rbf_morph_points)
    controls, displacements = _controls_and_displacements()
    output = domain.rbf_morph(controls, displacements, implementation="torch")
...
def test_domain_combined_rbf_morph_preserves_component_autograd():
    domain = _domain_with_coincident_points()
