...
class TestZarrReaderCache:
    ...
    def test_attribute_values_served_from_cache(self, zarr_dir_with_attrs):
        """A warm sample reads attribute values without touching the store."""
        from physicsnemo.datapipes.readers.zarr import ZarrReader

        cache = DatasetCache(ram_bytes_limit=2**24)
        reader = ZarrReader(zarr_dir_with_attrs, fields=["timestep"], cache=cache)
        import zarr

        first, _ = reader[0]
        # Mutate the store behind the cache; a warm read must not notice.
        zarr.open(reader._groups[0], mode="r+").attrs["timestep"] = -999.0
        second, _ = reader[0]
        assert torch.equal(first["timestep"], second["timestep"])
        assert first["timestep"].item() != -999.0
...
class TestVTKReaderCache:
    ...
    def test_file_resolution_key_needs_no_path_resolution(self, tmp_path, monkeypatch):
        """The cache identity is string-normalized; no stat per sample."""
        import pyvista as pv

        from physicsnemo.datapipes.readers.vtk import VTKReader

        case = tmp_path / "case_0"
        case.mkdir()
        pv.Sphere().extract_surface().triangulate().save(case / "geometry.stl")
        reader = VTKReader(tmp_path, cache=DatasetCache(ram_bytes_limit=2**24))
        monkeypatch.setattr(
            Path, "resolve", lambda self, strict=False: pytest.fail("resolve() called")
        )
        for _ in range(2):  # cold and warm lookups, neither may resolve()
            assert reader._get_file_by_extension(case, ".stl") == case / "geometry.stl"
