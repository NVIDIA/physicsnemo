...
class TestDomainMeshReaderCache:
    ...
    def test_missing_extra_boundary_is_not_cached(self, mesh_root, tmp_path):
        """An empty glob raises and is not remembered; adding the file fixes it."""
        cache = DatasetCache(ram_bytes_limit=2**24, disk_dir=tmp_path / "cache")
        reader = DomainMeshReader(
            mesh_root,
            extra_boundaries={"late": {"pattern": "late_*.pmsh"}},
            cache=cache,
        )
        with pytest.raises(FileNotFoundError):
            reader[0]
        two_triangles_2d.load().save(mesh_root / "run_0" / "late_geo.pmsh")
        dm, _ = reader[0]
        assert "late" in dm.boundary_names
