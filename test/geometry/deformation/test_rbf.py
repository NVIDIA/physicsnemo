...
from physicsnemo.geometry.deformation import radial_basis_function_deform
...
def test_radial_basis_function_deform_namespace_is_canonical():
    ...
    assert geometry.radial_basis_function_deform is radial_basis_function_deform
    ...
    assert not hasattr(Mesh, "radial_basis_function_deform")
    assert not hasattr(DomainMesh, "radial_basis_function_deform")


def test_radial_basis_function_deform_signature_is_introspectable():
    signature = inspect.signature(radial_basis_function_deform)
    ...
    assert get_type_hints(radial_basis_function_deform)["mesh"] == Mesh | DomainMesh


def test_mesh_radial_basis_function_deform_interpolates_controls_exactly():
    mesh = _triangle_mesh()
    controls, displacements = _controls_and_displacements()
    expected = mesh.points + displacements

    output = radial_basis_function_deform(
        mesh,
        controls,
        displacements,
        polynomial=True,
...
def test_mesh_radial_basis_function_deform_has_actionable_boundary_validation():
    ...
    with pytest.raises(TypeError, match="control_points"):
        radial_basis_function_deform(
            mesh, "handles", displacements, implementation="torch"
        )
    with pytest.raises(TypeError, match="control_displacements"):
        radial_basis_function_deform(mesh, controls, "motions", implementation="torch")
...
def test_domain_radial_basis_function_deform_resolves_common_nested_point_weight_path():
    ...
    nested = radial_basis_function_deform(
        domain,
        controls,
        displacements,
        point_weights=("motion", "weight"),
    ...
    flat = radial_basis_function_deform(
        domain,
        controls,
        displacements,
...
def test_domain_radial_basis_function_deform_clones_domain_global_data():
    ...
    output = radial_basis_function_deform(
        domain, controls, displacements, implementation="torch"
    )
...
def test_domain_radial_basis_function_deform_evaluates_combined_components_once(
    ...
    controls, displacements = _controls_and_displacements()
    expected = {
        "interior": radial_basis_function_deform(
            domain.interior,
            controls,
            displacements,
            point_weights="marker",
        ...
        **{
            name: radial_basis_function_deform(
                component,
                controls,
                displacements,
                point_weights="marker",
    ...
    original = point_module.radial_basis_function_deform_points
