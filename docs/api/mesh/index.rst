...
- **Tessellation**: triangulate polygon soups into simplicial meshes (convex
  ...
  ``Mesh.from_polygons``
- **Mesh generation**: fill a boundary ``Mesh`` with guaranteed-quality
  simplices (``tessellation.fill_interior``, exact boundaries and provenance
  data), or mesh an implicit domain — SDF, level set, or neural field — in
  any dimension on CPU or GPU (``generate.mesh_implicit_domain``, robust by
  construction and differentiable via ``generate.refit_mesh_to_implicit``);
